//@@ -1,6 +1,7 @@; (This line is the cause of most of my errors)
...
//checks if a move is invalid
const checkInvalid = (x, y, i, j) => (y+i < 0 || y+i > game.length-1 || x+j < 0 || x+j > game[0].length-1 || i+j === 0 || i === j)  // Proposed solution
...
// checks if a game is won at the end of a move
const checkGameWin = () => {
    for(let i = 0; i < game.length * game[0].length - 1; i++)
    {
    const x = i % game[0].length                                                        // proposed answer
